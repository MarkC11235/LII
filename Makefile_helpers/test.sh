...
for i in $test_files; do 
    ...
    # Step 1: Compile .cl to .cl_exe and capture output/errors
    COMPILE_OUTPUT=$(./liic $i $3 $4 2>&1)
    COMPILE_STATUS=$?
    
    if [ $COMPILE_STATUS -ne 0 ] || [ ! -f "${base_name}.cl_exe" ]; then
        # Compilation failed - write the error output to temp file
        echo "$COMPILE_OUTPUT" > "${i}.temp"
    else
        # Compilation succeeded - run the executable and capture output
        ./liivm "${base_name}.cl_exe" > "${i}.temp"
    ...
    # Compare output with expected (regardless of compilation success/failure)
    if diff -b -w "${i}.temp" "${i}.out" > /dev/null; then
        success_count=$((success_count + 1))
    else
        echo -e "\033[0;31mTest $i Failed\033[0m"
