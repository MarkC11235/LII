...
void parse_term(std::vector<Token>& tokens, Node* current){
    Token token = pop(tokens);
    ...
    if(token.get_type() == TokenType::NUMBER_TOKEN){
        Node* num = new Node(NodeType::NUM, token.get_value());
        factor->add_child(num);
    }
    else if(token.get_type() == TokenType::IDENTIFIER_TOKEN){
        Node* var = new Node(NodeType::VAR, token.get_value());
        factor->add_child(var);
        token = pop(tokens);
        //std::cout << "Checking value: " << token.get_value() << std::endl;
        if(token.get_type() == TokenType::OPENSQUAREBRACKET_TOKEN){
            token = pop(tokens);
            //std::cout << "Checking value: " << token.get_value() << std::endl;
            if(token.get_type() == TokenType::NUMBER_TOKEN || token.get_type() == TokenType::IDENTIFIER_TOKEN){
                var->add_value(token.get_value());
                //std::cout << "Adding value: " << token.get_value() << std::endl;
            }
            token = pop(tokens);
    ...
    else if(token.get_type() == TokenType::FUNCTION_TOKEN){
        parse_function_call(tokens, factor);
    }
    else {
        parsing_error("Syntax error: expected identifier or number", token);
    }

    if(peek(tokens).get_value() == "*" || peek(tokens).get_value() == "/"){
        Node* op = new Node(NodeType::OP, peek(tokens).get_value());
        current->add_child(op);
        pop(tokens);

        Node* term = new Node(NodeType::TERM, "");
        current->add_child(term);
        parse_term(tokens, term);
    }
}
...
void parse_expr(std::vector<Token>& tokens, Node* current){
    ...
    if(peek(tokens).get_value() == "+" || peek(tokens).get_value() == "-" || 
        peek(tokens).get_value() == "==" || peek(tokens).get_value() == "<" || 
        peek(tokens).get_value() == ">" || peek(tokens).get_value() == "<=" || 
        peek(tokens).get_value() == ">=" || peek(tokens).get_value() == "!="){
